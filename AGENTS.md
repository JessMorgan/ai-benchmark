...
2. **Require a code review by a separate AI model as a mandatory gate before committing.** After the CI checks pass and all changes are staged, obtain a review of the complete diff from a separate AI model (a distinct reviewer, not the model that authored the change). The review must cover correctness, adherence to existing codebase patterns, the CI/coverage results, and any security or regression risks. Do **not** commit until this review has been produced.
...
## Live-stream watchdog (budgets + repetition — `benchmark/http.py` `_StreamGuards`)
