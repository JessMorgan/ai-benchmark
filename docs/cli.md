...
The judge receives the original task prompt and response as explicitly delimited, quoted evaluation data, returns a validated 0–100 score with confidence, rationale, and criterion-level interpretations/evidence, and never changes the deterministic benchmark score or benchmark success status. The prompt tells the judge not to follow instructions in the candidate answer, emit tool calls, or continue the embedded task, and requires exactly one JSON object. HTTP judge requests stream each response so live reasoning/content progress is available and shutdown can close the active response immediately. They preserve each model's native thinking behavior, use a default `max_tokens` limit of 16384, and request a JSON object response by default; they do not apply a global thinking-token budget. Provider-specific thinking controls can be added through `judge.request_params`; see [Configuration Reference](configuration.md#semantic-judge-request-parameters). Judge input sidecars are retained automatically so interrupted runs can resume judging. The per-judge HTTP log records the exact merged request body, and each judge response metadata sidecar records whether the observed reasoning-token usage stayed within the requested budget when the provider exposes sufficient usage data. Each judge's raw JSON response is saved beside the benchmark response artifacts as `<plugin>.judge.<model>.txt`; the live TUI footer shows per-judge progress such as `Judging [Big Pickle: 4/17]`, and a `J` is appended to a plugin score once all configured judges have finished that response. While a judge source is still benchmarking, one judge-model slot is reserved for judging and benchmark capacity is reduced by one when the source limit is greater than one. As soon as that source's benchmark queue drains, judging expands to the source's full configured `model_thread_limit`. Judge threading mirrors target threading: `model_thread_limit` bounds how many distinct judge models run concurrently for a source (each active judge uses exactly one model slot), while `plugin_thread_limit` bounds how many cells each judge scores at once. Judges are run to completion in discovery order before another judge is loaded, so a single local judge stays resident instead of round-robin swapping between models. A single-thread source does not start judging until its benchmark work finishes.
