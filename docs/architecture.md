...
4. **Plugin Execution**
   ...
   - Pi tasks send a structured request to a fresh Node worker, receive text/thinking/tool events, and retain redacted gzip stdout/stderr only in debug mode.
...
New runs use `run.sqlite3` as the authoritative normalized store. The CLI
adopts an adjacent legacy `benchmark_state.json` into SQLite when needed;
`--storage json` remains an explicit compatibility path. Use
`--compare-storage JSON SQLITE` for a read-only semantic parity check and
`--measure-storage` for a reproducible synthetic size/latency baseline.

Compact storage keeps structured attempt/vote metadata and canonical compressed
payloads. Full HTTP, judge, OpenCode, and Pi transcripts are debug-only; when
`--debug-logs` or the `debug` profile is enabled, they are written as redacted
concatenated-member gzip logs. OpenCode/Pi subprocess output is therefore not
persisted as plaintext by the runtime.
